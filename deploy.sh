...
if ! (
    ...
    # What a link failure most likely means differs per platform, so the hint
    # names the cause of that platform rather than leaving the reader to sift
    # the log against a symptom that cannot occur on the machine in hand.
    case "${PLATFORM}" in
        windows) BUILD_HINT="A link failing on symbols such as '__imp___argc'
    means the compiler does not match the Qt installation." ;;
        macos)   BUILD_HINT="A link failing on a missing framework
    — 'AGL', for instance — means Qt ${QT_VERSION} predates the macOS SDK in use:
    Apple withdrew the framework, and this Qt still asks for it. Install a Qt
    built against that SDK, or an SDK of the version this Qt was tested with." ;;
        *)       BUILD_HINT="A link failing on a missing library
    means a development package of Qt or of one of its dependencies is missing." ;;
    esac

    die "the build failed — nothing was packaged
    The last lines above name the reason. ${BUILD_HINT}"
fi
...
# What is copied here carries the permissions the build gave it, and those follow
# the umask of the machine that built it. A restrictive umask therefore yields a
# bundle that only the account which built it may start, and the copy handed to
# someone else then refuses with a reason that names nothing of the cause.
#
# The permissions are set outright rather than left to the umask: everything is
# made readable, and 'X' — the capital one — adds the executable bit to
# directories and to what already carries one, so that data files do not gain an
# executable bit they have no use for.
#
# The '--' the other commands here are given is left out: the chmod of the BSD
# tools, which is the one macOS carries, does not know it.
chmod -R a+rX "${OUTPUT_DIR}"

# The executable is named once more on its own. The rule above grants no first
# executable bit to a plain file, and a build that wrote the binary without one
# would otherwise be packaged unstartable.
chmod a+rx "${APP_BINARY}"

info "permissions set, so that the bundle starts for whoever receives it"
...
case "${PLATFORM}" in
    windows) info "    ${APP_NAME}" ;;
    linux)   info "    ./genc3wb.sh"
             info "  which names the libraries and the plugins carried along." ;;
    macos)   info "    open ./${APP_NAME}"
             info "  Started so, what the application writes to standard output does not reach"
             info "  the terminal. Start the executable within the bundle to read that:"
             info "    ./${APP_NAME}/${APP_BINARY_REL}"
             info "  Where the machine refuses a bundle that came from another machine, remove"
             info "  the mark it was given on arrival:"
