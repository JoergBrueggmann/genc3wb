...
- Describe the preconditions of building genc³wb: a shell that runs bash, a Qt 6 found on the path or named by 'QMAKE', a compiler that knows the C++23 standard and a make, the deployment tool of Qt for the platform, and the tools that no step asks for yet that better the result.
- Name per operating system what is to be obtained, as a table, and give for Windows, for macOS and for Linux the commands that obtain it.
- Name the special preconditions of macOS: a Qt built against an SDK no older than the one Xcode carries, since an older Qt asks the linker for frameworks Apple has withdrawn; a Qt that carries the architecture that is built for; and the modules genc³wb asks for, which are those of 'qtbase' alone.
- Describe how the source is fetched and built, what 'deploy.sh' collects, where it leaves the result, and how that result is started on each of the three platforms.
- Describe the build that collects nothing, which is the one to make while developing, and how the test suite is run on its own with an offscreen platform.
...
- Name the likely cause of a failed link per platform, rather than the cause of Windows on every platform: a compiler that does not match the Qt installation on Windows, a framework the SDK in use no longer carries on macOS, and a missing development package on Linux. A deployment that failed on macOS pointed at a symbol of the entry point of Qt that cannot occur there, and named nothing of the cause.
- Set the permissions of the packaged bundle outright, rather than leaving them at what the umask of the building machine yields, without which a bundle built under a restrictive umask starts for the account that built it and for no other. Everything is made readable, and the executable bit is set on the directories, on what already carries one, and on the executable itself.
- Leave the '--' out of that command, which the chmod of the BSD tools does not know and which ended the deployment on macOS where it was given.
...
- Specify the *text increment* by which a code editor reports how its text changed, as FR-050 to FR-062, NFR-005, IR-015 and IR-016: its position, its range and its text, the *provided text* it is measured against, the *maximum idle time* after which it is provided, and the line it is written as.
- Change the 'Gc3CodeEditorWidget' to track every modification of its text and to derive one increment from the *provided text* and the text it currently holds, so that a receiver overwriting the range of that increment at its position with its text holds the same text as the editor.
- Carry a modification made by the *product* into the increment as well, and not only one made by the user, so that a file loaded into the editor reaches the receiver too and the *provided text* never diverges from what was reported.
- Provide the increment where the text was not modified for the maximum idle time, driven by a watchdog the code editor carries for that purpose, and take the text it currently holds as the *provided text* from then on.
- Divide the maximum idle time into ten ticks of the watchdog, so that the increment is provided after at most that time rather than after up to twice it.
- Provide no increment where the text the editor currently holds does not differ from its *provided text*, so that a pause in typing alone reports nothing.
- Write every increment provided to standard output as one line 'Position:<PPPP>, Range: <NNNN>, "<Text>"', the position and the range padded on the left with zeros to four digits.
- Require UTF-8 of what *product* writes to its own standard output, as IR-016, and set that encoding on the stream instead of leaving it at what the framework defaults to. Nothing demanded an encoding so far, and the bytes written were therefore a property of the version of Qt *product* is built with rather than of *product*: Qt 6 defaults to UTF-8, Qt 5 defaulted to the local 8-bit codec.
- Escape a backslash, a double quote, a line feed, a carriage return and a horizontal tab in the text of that line, without which an increment spanning several lines is not one line and cannot be read back.
- Announce every increment provided, so that a later version can hand it to the *compiler-compiler*.
- Separate the derivation of an increment from providing it, so that the derivation is free of side effects and can be asserted without standard output being written.
- Declare the increment, the two constants of the maximum idle time and the interface of the code editor in the design, and trace FR-050 to FR-062, NFR-005, IR-015 and IR-016 to them.
- Trace IR-001 to IR-014 in the design as well, which the design guideline demands of every interface requirement and which the design did not carry.
- Exclude from the scope of the specification the encoding in which *product* reads and writes a file, which is specified with the loading and the saving of an input file in a later version.
